...
from agents.macro_calculator_agent import get_macro_calculator_agent
...
def get_meal_planner_agent(session_id: str) -> ReActAgent:
    ...
    if session_id not in agent_store:
        # Initialize LLM
        llm = OpenAI(model="gpt-4o-mini", temperature=0) # Upgraded for better JSON
...
async def get_agent_response(message: str, session_id: str) -> AgentResponse:
    ...
    if route_name == "meal_planner":
        ...
        agent = get_macro_calculator_agent(session_id)
        response = await agent.run(message)
        raw_response = str(response)
    else:
